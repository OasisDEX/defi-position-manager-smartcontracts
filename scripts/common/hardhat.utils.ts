...
export class HardhatUtils {
  ...
  public logNetworkInfo() {
    console.log(
      `Network: ${this.hre.network.name}. Using addresses from ${this.targetNetwork}\n`
    );
  }
  ...
  public async deployContract<
    ...
    C extends Contract
  >(_factory: F | Promise<F>, params: Parameters<F["deploy"]>): Promise<C> {
  ...
  public async send(tokenAddr: string, to: string, amount: number) {
    const tokenContract = await this.hre.ethers.getContractAt(
      "IERC20",
      tokenAddr
    );
  ...
  public async setNewExchangeWrapper(acc: Signer, newAddr: string) {
    ...
    const registryInstance = await this.hre.ethers.getContractFactory(
      "SaverExchangeRegistry"
    );
    const registry = registryInstance.attach(
  ...
  public async getValidExecutionCallerOrOwner(
    signer: Signer,
    executor?: Contract
  ) {
    ...
    await this.sendEther(
      this.hre.ethers.provider.getSigner(0),
      executionOwner,
      "10"
    );
    return owner;
  ...
  public async getGasPrice(): Promise<EtherscanGasPrice["result"]> {
    ...
    const { data } = await axios.get<EtherscanGasPrice>(
