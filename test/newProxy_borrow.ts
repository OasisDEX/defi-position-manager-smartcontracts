...
import { AccountFactory, AccountImplementation, IDssProxyActions, ManagerLike } from "../typechain-types";
...
describe("Borrow - new Proxy", function () {
  ...
  async function deployFreshFactory() {
    ...
    const proxyAction = await ethers.getContractAt("IDssProxyActions", PROXY_ACTIONS_ADDRESS);
    ...
    await guard.setWhitelist(await proxyAction.address);
  ...
  async function executeProxy(proxy : AccountImplementation, methodName : any, args:any, overrides : any = undefined){
    
    const data = proxyAction.interface.encodeFunctionData(methodName,args);
    overrides?await proxy.execute(PROXY_ACTIONS_ADDRESS, data, overrides ):await proxy.execute(PROXY_ACTIONS_ADDRESS, data);
  }

  this.beforeAll(async function () {
    user1 = ethers.provider.getSigner(2);
    user2 = ethers.provider.getSigner(3);
    ({ guard, factory, dummy, proxyAction, cdpManager, mcdView }  = await loadFixture(deployFreshFactory));
  ...
  describe("create New vault", function () {
    ...
    it("Open of empty vault should not fail", async function () {
      ...
      await executeProxy(user1Proxy.connect(user1), "open", [CDP_MANAGER, ETH_A_ILK, await user1.getAddress()]);
    ...
    it("Open of non empty vault should not fail", async function () {
      ...
      const startBalance = ethers.utils.formatEther(await ethers.provider.getBalance(await user1.getAddress()));

      await executeProxy(user1Proxy.connect(user1), "openLockETHAndDraw", 
        [CDP_MANAGER, 
          MCD_JUG,
          ETH_JOIN,
          DAI_JOIN,
      ...
      const remainingBalance = ethers.utils.formatEther(await ethers.provider.getBalance(await user1.getAddress()));
